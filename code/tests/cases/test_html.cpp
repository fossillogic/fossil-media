...
FOSSIL_TEST_CASE(cpp_test_html_cpp_wrapper_load_and_root) {
    using fossil::media::Html;
    const std::string html = "<!DOCTYPE html><html><body><h1>Hello</h1></body></html>";
    Html doc = Html::from_string(html);
...
FOSSIL_TEST_CASE(cpp_test_html_cpp_wrapper_serialize_roundtrip) {
    using fossil::media::Html;
    const std::string html = "<div id=\"main\">Hello</div>";
    Html doc = Html::from_string(html);
    ASSUME_ITS_TRUE(doc.is_valid());
...
FOSSIL_TEST_CASE(cpp_test_html_cpp_wrapper_move_semantics) {
    using fossil::media::Html;
    const std::string html = "<span>MoveTest</span>";
    Html doc1 = Html::from_string(html);
    ASSUME_ITS_TRUE(doc1.is_valid());

    Html doc2 = std::move(doc1);
    ASSUME_ITS_TRUE(doc2.is_valid());
    ASSUME_ITS_FALSE(doc1.is_valid());

    fossil_media_html_node_t *root = doc2.root();
    ASSUME_NOT_CNULL(root);
    ASSUME_ITS_EQUAL_I32(fossil_media_html_node_type(root), FOSSIL_MEDIA_HTML_NODE_DOCUMENT);
}
